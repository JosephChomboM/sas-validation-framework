...
%macro _target_describe(data=, target=, byvar=, def_cld=0);
    ...
    %if &def_cld. ne 0 %then %do;
        ...
        proc fedsql sessref=conn noprint;
            create table casuser._tgt_data_filt {options replace=true} as
            select * from &data.
            where &byvar. <= &def_cld.;
        quit;
    %end;
    ...
    proc fedsql sessref=conn noprint;
        create table casuser._tgt_evolut_target {options replace=true} as
        select &byvar., count(*) as N, avg(&target.) as Mean
        from casuser._tgt_data_filt
        group by &byvar.;
    quit;

    /* Contar meses */
    proc sql noprint;
        select count(*) into :num_months trimmed from casuser._tgt_evolut_target;
    quit;
    ...
    %if &num_months. ne 1 %then %do;
        ...
        proc sql noprint;
            select Mean into :_tgt_means1 - :_tgt_means&num_months.
                from casuser._tgt_evolut_target;
        quit;
        ...
        /* Diferencia relativa */
        %if &first_mean. ne 0 %then
            %let relative_diff=%sysevalf((&last_mean. - &first_mean.) /
                &first_mean.);
    ...
    proc sgplot data=casuser._tgt_evolut_target;
        vline &byvar. / response=Mean markers
...
%macro _target_bandas(data=, data_type=, target=, byvar=, is_train=1);
    ...
    proc fedsql sessref=conn noprint;
        create table casuser._tgt_monthly {options replace=true} as
        select &byvar., avg(&target.) as avg_target
        from &data.
        group by &byvar.;
    quit;
    ...
    %if (&is_train.=1) or (%length(&_tgt_global_avg.)=0) %then %do;
        ...
        proc sql noprint;
            select std(avg_target) into :_tgt_std_monthly trimmed
                from casuser._tgt_monthly;
        quit;
    %end;
    %else %do;
    ...
    proc sgplot data=casuser._tgt_monthly subpixel noautolegend;
        ...
        series x=&byvar. y=avg_target / markers
            lineattrs=(color=blue thickness=2) legendlabel="RD" name="serie1";
...
%macro _target_ponderado_promedio(data=, data_type=, target=, monto=, byvar=,
    ...
    proc fedsql sessref=conn noprint;
        create table casuser._tgt_monthly_pond {options replace=true} as
        select &byvar.,
    ...
    %if (&is_train.=1) or (%length(&_tgt_global_avg_pond.)=0) %then %do;
        ...
        proc sql noprint;
            select std(avg_target_pond) into :_tgt_std_monthly_pond trimmed
                from casuser._tgt_monthly_pond;
        quit;
    %end;
    %else %do;
    ...
    %let min_val=%sysevalf(&_tgt_global_avg_pond. - 5 * &_tgt_std_monthly_pond.);
    %let max_val=%sysevalf(&_tgt_global_avg_pond. + 5 * &_tgt_std_monthly_pond.);
    ...
    proc sgplot data=casuser._tgt_monthly_pond subpixel noautolegend;
        band x=&byvar. lower=&inf. upper=&sup. / fillattrs=(color=graydd)
            legendlabel="+/- 2 Desv. Estandar" name="band1";
        series x=&byvar. y=avg_target_pond / markers
...
%macro _target_ponderado_suma(data=, data_type=, target=, monto=, byvar=,
    ...
    %global _tgt_global_sum_pond _tgt_std_sum_pond
        _tgt_global_ratio _tgt_std_ratio;
    %local inf sup inf_ratio sup_ratio min_ratio max_ratio;
    ...
    %if (&is_train.=1) or (%length(&_tgt_global_sum_pond.)=0) %then %do;
        proc sql noprint;
            select mean(sum_target_pond) into :_tgt_global_sum_pond trimmed
                from casuser._tgt_sum_pond;
            select std(sum_target_pond) into :_tgt_std_sum_pond trimmed
                from casuser._tgt_sum_pond;
        quit;
    %end;
    %else %do;
    ...
    proc sgplot data=casuser._tgt_sum_pond subpixel noautolegend;
        band x=&byvar. lower=&inf. upper=&sup. / fillattrs=(color=graydd)
            legendlabel="+/- 2 Desv. Estandar" name="band1";
        series x=&byvar. y=sum_target_pond / markers
    ...
    %if (&is_train.=1) or (%length(&_tgt_global_ratio.)=0) %then %do;
        proc sql noprint;
            ...
            select std(ratio_default_monto) into :_tgt_std_ratio trimmed
                from casuser._tgt_ratio;
        quit;
    %end;
    %else %do;
