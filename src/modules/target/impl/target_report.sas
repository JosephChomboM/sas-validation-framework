...
%macro _target_plot_series(data=, yvar=, ylabel=, file_suffix=,
...
%macro _target_report_rd(file_prefix=);
    ...
    %_target_plot_series(data=casuser._tgt_rd_monthly, yvar=RD, ylabel=RD,
...
%macro _target_plot_bands(data=, yvar=, refvar=, lower=, upper=, axismin=,
    axismax=, ylabel=, ref_color=red, file_suffix=, title_txt=,
    file_prefix=);
...
%macro _target_report_weight_sum(file_prefix=);
    ...
    %_target_plot_bands(data=casuser._tgt_weight_sum, yvar=Sum_Target_Pond,
        refvar=Global_Sum, lower=Lower_Band, upper=Upper_Band,
        axismin=, axismax=, ylabel=RD Pond. por Suma de Monto,
