...
========================================================================= */
%macro calibracion_contract(input_caslib=, train_table=, oot_table=, vars_num=,
    vars_cat=, target=, score_var=, byvar=, def_cld=, monto_var=);

    %let _cal_rc=0;

    %local _cal_nobs_trn _cal_nobs_oot _cal_has_col _cal_nobs_trn_filt
        _cal_nobs_oot_filt;
    ...
    %if &_cal_nobs_trn.=0 %then %do;
        %put ERROR: [calibracion_contract] TRAIN vacio o no accesible.;
        %let _cal_rc=1;
        %return;
    %end;

    %if &_cal_nobs_oot.=0 %then %do;
        %put ERROR: [calibracion_contract] OOT vacio o no accesible.;
        %let _cal_rc=1;
        %return;
    %end;
    ...
    %_cal_chk(mem=&train_table., col=&target., req=1);
    %if &_cal_rc.=1 %then %return;
    %_cal_chk(mem=&oot_table., col=&target., req=1);
    %if &_cal_rc.=1 %then %return;

    %_cal_chk(mem=&train_table., col=&score_var., req=1);
    %if &_cal_rc.=1 %then %return;
    %_cal_chk(mem=&oot_table., col=&score_var., req=1);
    %if &_cal_rc.=1 %then %return;

    %_cal_chk(mem=&train_table., col=&byvar., req=1);
    %if &_cal_rc.=1 %then %return;
    %_cal_chk(mem=&oot_table., col=&byvar., req=1);
    %if &_cal_rc.=1 %then %return;

    %if %length(%superq(monto_var)) > 0 %then %do;
        %_cal_chk(mem=&train_table., col=&monto_var., req=0);
        %_cal_chk(mem=&oot_table., col=&monto_var., req=0);
    %end;
    ...
    %if &_cal_nobs_trn_filt.=0 %then %do;
        %put ERROR: [calibracion_contract] TRAIN no tiene observaciones con
            &byvar. <= &def_cld..;
        %let _cal_rc=1;
        %return;
    %end;
    ...
    %if &_cal_nobs_oot_filt.=0 %then %do;
        %put ERROR: [calibracion_contract] OOT no tiene observaciones con
            &byvar. <= &def_cld..;
        %let _cal_rc=1;
        %return;
    %end;

    %put NOTE: [calibracion_contract] OK - TRAIN=&_cal_nobs_trn. obs,
    ...
    %put NOTE: [calibracion_contract] target=&target. score=&score_var.
        byvar=&byvar. def_cld=&def_cld. monto=&monto_var..;
