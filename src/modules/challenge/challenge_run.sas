/* =========================================================================
challenge_run.sas - Macro publica del modulo Challenge (METOD9)
========================================================================= */
...
%macro challenge_run(input_caslib=PROC, train_table=_train_input,
    ...
    %let _chall_score_mode=%upcase(%superq(chall_score_source));
    %if %length(%superq(_chall_score_mode))=0 %then %let _chall_score_mode=AUTO;
    %let _chall_mode_eff=%upcase(%superq(chall_mode));
    ...
    %if &_chall_mode_eff.=CUSTOM %then %do;
        %if %length(%superq(chall_custom_vars_num)) > 0 or
            %length(%superq(chall_custom_vars_cat)) > 0 %then %do;
            %let _chall_vars_num_cfg=&chall_custom_vars_num.;
            %let _chall_vars_cat_cfg=&chall_custom_vars_cat.;
            %let _chall_custom_mode=1;
            %if %length(%superq(chall_custom_target)) > 0 %then
                %let _chall_target_cfg=&chall_custom_target.;
            %put NOTE: [challenge_run] Modo CUSTOM activado.;
        %end;
        %else %do;
            %put WARNING: [challenge_run] chall_mode=CUSTOM sin listas custom.
    ...
    %else %if &_chall_score_mode.=CUSTOM %then %do;
        %if %length(%superq(chall_custom_score_var)) > 0 %then
            %let _chall_score=&chall_custom_score_var.;
    %end;
    ...
    %if &_chall_rc. ne 0 %then %do;
        %put ERROR: [challenge_run] Contract fallido. Challenge abortado.;
        %return;
    %end;
    ...
    %let _chall_tbl_prefix=chal_t&troncal_id._&_chall_scope_abbr.;
    ...
    %let _chall_partition_pct=&chall_partition_pct.;
    ...
    %let _chall_penalty_lambda=&chall_penalty_lambda.;
    ...
    %let _chall_gb_stagnation=&chall_gb_stagnation.;
    %if %length(%superq(_chall_gb_stagnation))=0 %then
        %let _chall_gb_stagnation=0;
    %let _chall_ntrees_cap=&chall_gb_ntrees_cap.;
    ...
    %let _chall_top_k=&chall_top_k.;
    %if %length(%superq(_chall_top_k))=0 %then %let _chall_top_k=40;
    %let _chall_top_models=&chall_top_models.;
    ...
    %_challenge_report(benchmark_data=work._chall_benchmark_global,
        ...
        file_prefix=&_chall_report_prefix.);
