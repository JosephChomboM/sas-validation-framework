...
%macro missings_run( input_caslib=PROC, train_table=_train_input,
    ...
    %local _miss_vars_num _miss_vars_cat _miss_threshold _report_path
        _images_path _file_prefix _scope_abbr _miss_is_custom _seg_num;
    ...
    /* Resolver threshold */
    %if %length(%superq(miss_threshold)) > 0 %then %let
        _miss_threshold=&miss_threshold.;
    ...
    %if &_miss_is_custom.=0 %then %do;
        ...
        %if %substr(&scope., 1, 3)=seg %then %do;
            ...
            proc sql noprint;
                select strip(num_list) into :_miss_vars_num trimmed from
                    casuser.cfg_segmentos where troncal_id=&troncal_id. and
                    seg_id=&_seg_num.;

                select strip(cat_list) into :_miss_vars_cat trimmed from
                    casuser.cfg_segmentos where troncal_id=&troncal_id. and
                    seg_id=&_seg_num.;
            quit;
        %end;
        ...
        %if %length(%superq(_miss_vars_num))=0 %then %do;
            proc sql noprint;
                select strip(num_unv) into :_miss_vars_num trimmed from
        ...
        %if %length(%superq(_miss_vars_cat))=0 %then %do;
            proc sql noprint;
                select strip(cat_unv) into :_miss_vars_cat trimmed from
                    casuser.cfg_troncales where troncal_id=&troncal_id.;
            quit;
        %end;
    %end;
    ...
    ================================================================== */
    %missings_contract( input_caslib=&input_caslib., train_table=&train_table.,
        oot_table=&oot_table., vars_num=&_miss_vars_num.,
    ...
    ================================================================== */
    %_missings_report( input_caslib=&input_caslib., train_table=&train_table.,
        oot_table=&oot_table., vars_num=&_miss_vars_num.,
