...
AUTO   - resuelve vars desde config (var_num_list/var_cat_list)
...
%macro missings_run( input_caslib=PROC, train_table=_train_input,
    ...
    %local _miss_vars_num _miss_vars_cat _miss_threshold _report_path
        _images_path _file_prefix _scope_abbr _miss_is_custom;
    ...
    %if &_miss_is_custom.=0 %then %do;
        ...
        %if %substr(&scope., 1, 3)=seg %then %do;
            ...
            proc sql noprint;
                select strip(var_num_list) into :_miss_vars_num trimmed from
                    casuser.cfg_segmentos where troncal_id=&troncal_id. and
                    seg_id=&_seg_num.;
                select strip(var_cat_list) into :_miss_vars_cat trimmed from
                    casuser.cfg_segmentos where troncal_id=&troncal_id. and
                    seg_id=&_seg_num.;
            quit;
        %end;
