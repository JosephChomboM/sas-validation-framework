/* =========================================================================
decision_tree_report.sas - Reportes ODS para DT Challenge (METOD9)
========================================================================= */
...
%macro _decision_tree_report(benchmark_data=, eval_data=, topk_data=, topn_data=,
    monthly_data=, registry_data=, champion_data=, report_path=, images_path=,
    file_prefix=, model_type=APP);

    %local _chall_band_thres _chall_band_good;

    %let _chall_band_thres=0.4;
    %if %upcase(%superq(model_type))=BHV %then %let _chall_band_thres=0.5;
    %let _chall_band_good=%sysevalf(&_chall_band_thres. + 0.1);
    ...
    proc report data=&eval_data. nowd missing;
        columns Iteration GiniCoefficient CRIT NBINS MAXLEVEL LEAFSIZE;
        define Iteration / display "Iter";
        define GiniCoefficient / display "GINI Tune" format=8.4;
        define CRIT / display "Criterion";
    ...
    proc report data=&topk_data. nowd missing;
        columns Model_Rank Gini_Train Gini_OOT Gini_Penalizado CRIT NBINS
            MAXLEVEL LEAFSIZE MAXBRANCH MISSING BINMETHOD PRUNE;
        ...
        define MAXLEVEL / display "MaxDepth";
        define LEAFSIZE / display "LeafSize";
        define MAXBRANCH / display "MaxBranch";
        define MISSING / display "Missing";
        define BINMETHOD / display "BinMethod";
        define PRUNE / display "Prune";
    run;
    title;
    ...
    data work._chall_plot_monthly;
        set &monthly_data.;
        Band_Low=0;
        Band_Warn=&_chall_band_thres.;
        Band_Good=&_chall_band_good.;
        Band_Top=1;
    run;
    ...
    proc sgplot data=work._chall_plot_monthly subpixel;
        ...
        band x=Periodo lower=Band_Low upper=Band_Warn /
            fillattrs=(color=cxF4CCCC) transparency=0.35;
        band x=Periodo lower=Band_Warn upper=Band_Good /
            fillattrs=(color=cxFFF2CC) transparency=0.35;
        band x=Periodo lower=Band_Good upper=Band_Top /
    ...
    title "Challenge - GINI Penalizado Final";
        ...
        xaxis label="Rank Final" type=discrete;
    run;
    title;
    ods graphics / reset=all;
    ...
    title "Challenge - Autotune History";
        ...
        yaxis label="GINI";
        xaxis label="Iteracion" type=discrete;
    run;
    title;
    ods graphics / reset=all;
