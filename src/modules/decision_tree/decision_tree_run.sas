/* =========================================================================
decision_tree_run.sas - Macro publica del modulo Decision Tree
========================================================================= */
%include "&fw_root./src/modules/decision_tree/decision_tree_contract.sas";
...
%include "&fw_root./src/modules/decision_tree/impl/decision_tree_report.sas";

%macro decision_tree_run(input_caslib=PROC, train_table=_train_input,
    ...
    %local _chall_target_cfg _chall_pd _chall_xb _chall_score _chall_byvar_cfg
        ...
        _chall_custom_mode _chall_nparallel _chall_maxdepth_cap
        _chall_numbin_cap;
    ...
    %put NOTE:======================================================;
    %put NOTE: [decision_tree_run] INICIO - troncal=&troncal_id.
        scope=&scope.;
    %put NOTE:======================================================;
    ...
    %let _chall_score_mode=%upcase(%superq(dt_score_source));
    %if %length(%superq(_chall_score_mode))=0 %then %let _chall_score_mode=AUTO;
    %let _chall_mode_eff=%upcase(%superq(dt_mode));
    ...
    %if &_chall_mode_eff.=CUSTOM %then %do;
        %if %length(%superq(dt_custom_vars_num)) > 0 or
            %length(%superq(dt_custom_vars_cat)) > 0 %then %do;
            %let _chall_vars_num_cfg=&dt_custom_vars_num.;
            %let _chall_vars_cat_cfg=&dt_custom_vars_cat.;
            %let _chall_custom_mode=1;
            %if %length(%superq(dt_custom_target)) > 0 %then
                %let _chall_target_cfg=&dt_custom_target.;
            %put NOTE: [decision_tree_run] Modo CUSTOM activado.;
        %end;
        %else %do;
            %put WARNING: [decision_tree_run] dt_mode=CUSTOM sin listas custom.
    ...
    %else %if &_chall_score_mode.=CUSTOM %then %do;
        %if %length(%superq(dt_custom_score_var)) > 0 %then
            %let _chall_score=&dt_custom_score_var.;
    %end;
    ...
    %decision_tree_contract(input_caslib=&input_caslib.,
    ...
    %if &_chall_rc. ne 0 %then %do;
        %put ERROR: [decision_tree_run] Contract fallido.
            Decision Tree abortado.;
        %return;
    %end;
    ...
    %let _chall_report_prefix=decision_tree_troncal_&troncal_id._&_chall_scope_abbr.;
    %let _chall_tbl_prefix=dt_t&troncal_id._&_chall_scope_abbr.;
    ...
    %let _chall_astore_name=&_chall_tbl_prefix._dt_chmp;
    ...
    %let _chall_partition_pct=&dt_partition_pct.;
    ...
    %let _chall_penalty_lambda=&dt_penalty_lambda.;
    ...
    %let _chall_maxdepth_cap=&dt_maxdepth_cap.;
    %if %length(%superq(_chall_maxdepth_cap))=0 %then
        %let _chall_maxdepth_cap=12;
    %let _chall_numbin_cap=&dt_numbin_cap.;
    %if %length(%superq(_chall_numbin_cap))=0 %then
        %let _chall_numbin_cap=100;

    %_chall_dt_set_ranges(data=work._chall_train_part, num=&_chall_vars_num.,
        cat=&_chall_vars_cat., target=&_chall_target.,
        maxdepth_cap=&_chall_maxdepth_cap.,
        numbin_cap=&_chall_numbin_cap.);

    %let _chall_nparallel=5;

    %_chall_dt_tune(data=casuser._chall_train_part, num_input=&_chall_vars_num.,
    ...
    %let _chall_top_k=&dt_top_k.;
    %if %length(%superq(_chall_top_k))=0 %then %let _chall_top_k=40;
    %let _chall_top_models=&dt_top_models.;
    ...
    %_chall_build_registry(results=work._chall_topn_rank, troncal_id=&troncal_id.,
        ...
        algo_name=Decision Tree, algo_code=dt,
    ...
    %_decision_tree_report(benchmark_data=work._chall_benchmark_global,
    ...
    %put NOTE:======================================================;
    %put NOTE: [decision_tree_run] FIN - &_chall_report_prefix.;
    %put NOTE:======================================================;
%mend decision_tree_run;
