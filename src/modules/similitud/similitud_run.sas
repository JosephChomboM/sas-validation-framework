...
%macro similitud_run( input_caslib=PROC, train_table=_train_input,
    ...
    proc sql noprint;
        select strip(target) into :_simil_target trimmed
        from casuser.cfg_troncales
        where troncal_id = &troncal_id.;

        select strip(byvar) into :_simil_byvar trimmed
        from casuser.cfg_troncales
        where troncal_id = &troncal_id.;
    quit;
    ...
    %else %do;
        ...
        %put NOTE: [similitud_run] Output -> reports/METOD4.2/ +
            images/METOD4.2/;
    %end;
    ...
    ================================================================== */
    %_similitud_report( input_caslib=&input_caslib.,
        train_table=&train_table., oot_table=&oot_table.,
        target=&_simil_target., byvar=&_simil_byvar.,
        vars_num=&_simil_vars_num., vars_cat=&_simil_vars_cat.,
