/* =========================================================================
random_forest_parallel.sas - Paralelizacion y refit para RF Challenge
========================================================================= */
...
%macro _chall_worker_refit(phase=TOPK, session_index=1, shard_table=,
    ...
    %local _phase_uc _n_models _m _cfg_id _ntree _mtry _bootstrap _maxlevel
        _nbins _leafsize _maxbranch _missing _binmethod _grow _predvar
        _g_penal _worker_sess;
    ...
    %do _m=1 %to &_n_models.;
        data _null_;
            ...
            call symputx('_bootstrap', bootstrap, 'L');
            ...
            call symputx('_grow', grow, 'L');
        run;

        ods exclude all;
        proc forest data=casuser._chall_train_part
            assignmissing=&_missing.
            binmethod=&_binmethod.
            inbagfraction=&_bootstrap.
            ...
            savestate rstore=casuser._chall_rf_w&session_index._cfg&_cfg_id.;
        run;

        proc astore;
            score data=casuser.&_chall_train_table.
                out=casuser._chall_trn_scd_w&session_index._cfg&_cfg_id.
                rstore=casuser._chall_rf_w&session_index._cfg&_cfg_id.
        ...
        %if &_phase_uc.=TOPN %then %do;
            ...
            %_chall_gini_monthly(data=work._chall_full_scored,
                ...
                model_label=RF_&_cfg_id., algo_name=RandomForest,
        ...
        %if &persist_champion.=1 and &_cfg_id.=1 %then %do;
            %_save_into_caslib(m_cas_sess_name=&_worker_sess.,
                ...
                m_input_data=_chall_rf_w&session_index._cfg&_cfg_id.,
                m_output_caslib=OUT,
