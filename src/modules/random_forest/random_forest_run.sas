/* =========================================================================
random_forest_run.sas - Macro publica del modulo Random Forest
========================================================================= */
%include "&fw_root./src/modules/random_forest/random_forest_contract.sas";
%include "&fw_root./src/modules/challenge/impl/challenge_prechallenge.sas";
%include "&fw_root./src/modules/random_forest/impl/random_forest_compute.sas";
%include "&fw_root./src/modules/random_forest/impl/random_forest_parallel.sas";
...
%include "&fw_root./src/modules/random_forest/impl/random_forest_report.sas";

%macro random_forest_run(input_caslib=PROC, train_table=_train_input,
    ...
    %global _chall_rc _chall_target _chall_byvar _chall_vars_num
        ...
        _chall_penalty_lambda _chall_run_root
    ...
    %put NOTE:======================================================;
    %put NOTE: [random_forest_run] INICIO - troncal=&troncal_id.
        scope=&scope.;
    %put NOTE:======================================================;
    ...
    %let _chall_score_mode=%upcase(%superq(rf_score_source));
    %if %length(%superq(_chall_score_mode))=0 %then %let _chall_score_mode=AUTO;
    %let _chall_mode_eff=%upcase(%superq(rf_mode));
    ...
    %if &_chall_mode_eff.=CUSTOM %then %do;
        %if %length(%superq(rf_custom_vars_num)) > 0 or
            %length(%superq(rf_custom_vars_cat)) > 0 %then %do;
            %let _chall_vars_num_cfg=&rf_custom_vars_num.;
            %let _chall_vars_cat_cfg=&rf_custom_vars_cat.;
            %let _chall_custom_mode=1;
            %if %length(%superq(rf_custom_target)) > 0 %then
                %let _chall_target_cfg=&rf_custom_target.;
            %put NOTE: [random_forest_run] Modo CUSTOM activado.;
        %end;
        %else %do;
            %put WARNING: [random_forest_run] rf_mode=CUSTOM sin listas custom.
    ...
    %else %if &_chall_score_mode.=CUSTOM %then %do;
        %if %length(%superq(rf_custom_score_var)) > 0 %then
            %let _chall_score=&rf_custom_score_var.;
    %end;
    ...
    %random_forest_contract(input_caslib=&input_caslib.,
    ...
    %if &_chall_rc. ne 0 %then %do;
        %put ERROR: [random_forest_run] Contract fallido.
            Random Forest abortado.;
        %return;
    %end;
    ...
    %let _chall_report_prefix=random_forest_troncal_&troncal_id._&_chall_scope_abbr.;
    %let _chall_tbl_prefix=rf_t&troncal_id._&_chall_scope_abbr.;
    ...
    %let _chall_astore_name=&_chall_tbl_prefix._rf_chmp;
    ...
    %let _chall_partition_pct=&rf_partition_pct.;
    ...
    %let _chall_penalty_lambda=&rf_penalty_lambda.;
    ...
    %let _chall_ntrees_cap=&rf_ntrees_cap.;
    %if %length(%superq(_chall_ntrees_cap))=0 %then %let _chall_ntrees_cap=350;
    %let _chall_maxdepth_cap=&rf_maxdepth_cap.;
    %if %length(%superq(_chall_maxdepth_cap))=0 %then
        %let _chall_maxdepth_cap=30;

    %_chall_rf_set_ranges(data=work._chall_train_part, num=&_chall_vars_num.,
        cat=&_chall_vars_cat., target=&_chall_target.,
        ntrees_cap=&_chall_ntrees_cap.,
        maxdepth_cap=&_chall_maxdepth_cap.);

    %let _chall_nparallel=5;

    %_chall_rf_tune(data=casuser._chall_train_part, num_input=&_chall_vars_num.,
        cat_input=&_chall_vars_cat., target_input=&_chall_target.,
        nparallel=&_chall_nparallel., eval_out=work._chall_eval_history,
    ...
    %let _chall_top_k=&rf_top_k.;
    %if %length(%superq(_chall_top_k))=0 %then %let _chall_top_k=40;
    %let _chall_top_models=&rf_top_models.;
    ...
    %_chall_build_registry(results=work._chall_topn_rank, troncal_id=&troncal_id.,
        ...
        algo_name=Random Forest, algo_code=rf,
    ...
    %_random_forest_report(benchmark_data=work._chall_benchmark_global,
    ...
    %put NOTE:======================================================;
    %put NOTE: [random_forest_run] FIN - &_chall_report_prefix.;
    %put NOTE:======================================================;
%mend random_forest_run;
