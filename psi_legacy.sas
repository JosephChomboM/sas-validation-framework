...
%macro _psi_calc(dev=, oot=, var=, n_buckets=10, flg_continue=1);
    ...
    proc sql noprint;
        /* Frecuencias DEV */
        create table _freq_dev_&rnd. as
        select bucket, count(*) as n_dev
        from _dev_bucket_&rnd.
        group by bucket;

        /* Frecuencias OOT */
        create table _freq_oot_&rnd. as
        select bucket, count(*) as n_oot
        from _oot_bucket_&rnd.
        group by bucket;

        /* Totales */
        select coalesce(sum(n_dev), 0) into :tot_dev trimmed from _freq_dev_&rnd.;
        select coalesce(sum(n_oot), 0) into :tot_oot trimmed from _freq_oot_&rnd.;
    quit;
    ...
    %if &tot_dev. > 0 and &tot_oot. > 0 %then %do;
        proc sql noprint;
            /* Unir y calcular PSI */
            create table _psi_calc_&rnd. as
            select 
                coalesce(a.bucket, b.bucket) as bucket,
                coalesce(a.n_dev, 0) as n_dev,
                coalesce(b.n_oot, 0) as n_oot,
            ...
            from _freq_dev_&rnd. a
            full join _freq_oot_&rnd. b
                on a.bucket = b.bucket;

            /* Calcular PSI total */
            select sum((pct_oot - pct_dev) * log(pct_oot / pct_dev)) 
                into :psi_valor trimmed
            from _psi_calc_&rnd.;
        quit;
    %end;
    ...
    proc datasets lib=work nolist nowarn;
        delete _rank_dev_&rnd. _cortes_&rnd. _dev_bucket_&rnd. 
               _oot_bucket_&rnd. _freq_dev_&rnd. _freq_oot_&rnd. _psi_calc_&rnd.;
    quit;
...
%macro __psi_variables(
    ...
    %do %while(%length(&v.) > 0);
        ...
        %if &mensual. = 1 and &n_meses. > 0 %then %do;
            ...
            %do %while(%length(&c.) > 0);
                ...
                /* Agregar al cubo */
                proc sql;
                    insert into cubo_psi (Variable, &byvar, PSI, Tipo)
    ...
    %end;
        ...
        %if &hay_mensual. > 0 %then %do;
            ...
            proc sql;
                create table _cubo_psi_wide_tmp as
                select a.*, b.PSI as PSI_Total format=10.6
                from cubo_psi_wide a
                left join cubo_psi(where=(Tipo="Total")) b
...
/*--------------------------------------------------------------
  Macro: Heatmap de estabilidad
--------------------------------------------------------------*/
%macro __plot_psi_heatmap(data=cubo_psi);

    %local n_mensual;
    
    proc sql noprint;
        select count(*) into :n_mensual trimmed
        from &data.
        where Tipo = "Mensual";
    quit;

    %if &n_mensual. = 0 %then %do;
        %put NOTE: No hay datos mensuales para el heatmap.;
        %return;
    %end;

    data _heatmap_temp;
        set &data.(where=(Tipo="Mensual"));
    ...
    proc sgplot data=_heatmap_temp;
        heatmapparm x=&byvar y=Variable colorresponse=Semaforo /
            colormodel=(lightgreen yellow lightcoral)
            outline outlineattrs=(color=gray);
        text x=&byvar y=Variable text=PSI_Label /
            textattrs=(size=8 weight=bold);
        xaxis label="&byvar" valueattrs=(size=9);
    ...
    proc datasets lib=work nolist nowarn;
        delete _heatmap_temp;
    quit;

%mend __plot_psi_heatmap;


/*--------------------------------------------------------------
  Macro: Gráfico resumen todas las variables
--------------------------------------------------------------*/
%macro __plot_psi_resumen(data=psi_resumen);
...
%macro __psi_report(
	...
	proc format;
		value PsiSignif -0.0-<0.1  = "lightgreen"
							0.1-<0.25 = "yellow"
							0.25-<9999 = "red"
		;
	run;
    /* Crear Excel */
    ods excel file="&&path_troncal_&tr./&_excel_path./tro_&tr._seg_&seg._PSI.xlsx"
        options(embedded_titles="yes" embedded_footnotes="yes");

    /*--- HOJA 1: CUBO_PSI ---*/
    ods excel options(sheet_name="PSI" sheet_interval="none");
    title "CUBO PSI: Detalle por Variable y Periodo";
    footnote "Tipo: Mensual = PSI de ese mes vs TRAIN | Total = PSI OOT completo vs TRAIN";

    proc print data=cubo_psi noobs label style(column)={backgroundcolor= PsiSignif.};
    ...
    proc print data=cubo_psi_wide noobs style(column)={backgroundcolor= PsiSignif.};
    run;
    title;
...
%macro verify_psi(dataset1, dataset2);
    ...
    %if &nobs1. = 0 %then %do;
        %put WARNING DEVELOPER: [PSI] Dataset TRAIN sin observaciones: &dataset1.;
        %return;
    %end;
    ...
    %if %length(&_var_time.) > 0 and &_var_time. ne . %then %do;
        ...
        %let dsid1 = %sysfunc(open(&dataset1.));
        %if &dsid1. > 0 %then %do;
            %if %sysfunc(varnum(&dsid1., &_var_time.)) > 0 %then %let has_byvar = 1;
            %let rc = %sysfunc(close(&dsid1.));
        %end;
        ...
        %if &has_byvar. = 1 %then %do;
            %let dsid2 = %sysfunc(open(&dataset2.));
            %if &dsid2. > 0 %then %do;
                %if %sysfunc(varnum(&dsid2., &_var_time.)) = 0 %then %let has_byvar = 0;
                %let rc = %sysfunc(close(&dsid2.));
            %end;
        %end;
    %end;
    ...
    %if &has_byvar. = 0 %then %do;
        %put WARNING DEVELOPER: [PSI] Variable de tiempo no definida o no existe en datasets: &_var_time.;
        %return;
    %end;
    ...
    %if &has_num. = 0 and &has_cat. = 0 %then %do;
        %put WARNING DEVELOPER: [PSI] No hay variables numéricas ni categóricas definidas.;
        %return;
    %end;
